...
if(!user){  //if employee doesnt exist
    return res.status(204).json({"message":`No user ID matches ${req.params.id}`});
}
res.json(user);
 //#endregion
//#region In-Memory
...
const updateUserPwd=async (req,res)=>{
    ...
    if(!user)return res.status(204).json({"message":`No user ID matches ${req.body.id}`});
    try{
    if(req?.body?.oldpassword || req?.body?.newpassword){
        const match=await bcrypt.compare(req?.body?.oldpassword,user.password);
        if(!match)return res.status(204).json({"message":`password: ${req.body.id} do not match `})
        else{
            
            const hashedPwd=await bcrypt.hash(req?.body?.newpassword, 10); //encrypt an sort
